...
A Doubly robust estimator for the ATE is the Augmented inverse probability weighted AIPW estimator: 
\[
\frac{1}{n}\sum_{i = 1}^n\frac{Y_i X_i}{\hat{p}_i} - \frac{\hat{T}\{Y_i | X_i = 1, \boldsymbol{Z}_i\}(X_i - \hat{p}_i)}{\hat{p}_i} - \frac{Y_i (1 - X_i)}{1 - \hat{p}_i} - \frac{\hat{T}\{Y_i | X_i = 0, \boldsymbol{Z}_i\}(X_i - \hat{p}_i)}{1 - \hat{p}_i}.  
\]
Any consistent estimation method can be used for the outcome and exposure models. As long as either the outcome model or the propensity score model is correctly specified for confounding, then this doubly robust estimator is consistent for the ATE. 
...
\begin{tikzpicture}[->,>=stealth,shorten >=2pt,auto,node distance=2.5cm,
                    thick,main node/.style={circle,fill=black,inner sep=0pt,minimum size=6pt},
                    grey node/.style={circle,fill=gray!50,inner sep=0pt,minimum size=6pt}]
  % Nodes
  ...
  \node[below] at (1) {X};
  \node[below] at (2) {Y};
  \node[above] at (3) {$\boldsymbol{Z}$};

  % Edges
  \path[every node/.style={font=\sffamily\small}]
    (1) edge[->, line width=1.2pt] (2);
  \path[every node/.style={font=\sffamily\small}]
    (3) edge[->, line width=1.2pt] (1);
  \path[every node/.style={font=\sffamily\small}]
    (3) edge[->, line width=1.2pt] (2);        
\end{tikzpicture}
```
