...
#' d <- 5
#' standardize_glm(
#'   formula.outcome = Oesophagealcancer ~ (Everhotbev + Age + Dial + Samsu + Cigs)^2,
...
#' data <- AF::clslowbwt
#' x <- standardize_glm(
#'   formula.outcome = bwt ~ smoker * (race + age + lwt) + I(age^2) + I(lwt^2),
...
standardize_glm <- function(formula.outcome,
  ...
  if (any(is.na(mnams))) {
    stop(paste0(
      "variable(s) ", paste(xnms[which(is.na(mnams))], collapse = ", "),
  ...
  if (is.null(formula.exposure)) {
    ...
    EE.means <- weights * (predmat - matrix(rep(estimates, each = n),
      nrow = n,
      ncol = nrow(valuesout)
    ...
    lower.I <- cbind(matrix(0,
      nrow = length(fit.outcome$coefficients),
      ncol = nrow(valuesout)
    ), sandwich.fit$I)
    ...
    Lterm1 <- (1 / n) * matrix(rdot1 * (1 - exposure / g.weights),
      nrow = 1, ncol = n
    ) %*% model.matrix.outcome.exposed
    Lterm0 <- (1 / n) * matrix(rdot0 * ((1 - exposure) / (1 - g.weights) - 1),
      nrow = 1, ncol = n
    ) %*% model.matrix.outcome.unexposed
