...
test_that("check estimates and standard errors standardize_glm (case-control estimator)", {
  ...
  expect_equal(x$res_contrast$V1$estimates$se, c(1.68549810412155e-05, 0.000218937483527179), tolerance = 1e-5)
})
...
test_that("check estimates and standard errors standardize_glm (dr estimator)", {
  ...
  expect_equal(x$res_contrast$V2$est_table$`lower 0.95`[2], -424.6136, tolerance = 1e-5)
  expect_equal(x$res_contrast$V2$est_table$`upper 0.95`[2], -22.7335, tolerance = 1e-5)
})
