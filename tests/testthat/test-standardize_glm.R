test_that("bad names gives error", {
  ...
  dd <- data.frame(Z, X, Y)
  expect_error(standardize_glm(formula_outcome = Y ~ X + Z + I(X^2), data = dd, values = list(X = 1, blue = 2)))
})
...
test_that("non-binary exposure gives error when using the doubly robust estimator", {
  ...
  Y <- rnorm(n, mean = X + Z + 0.1 * X^2)
  dd <- data.frame(Z, X, Y)
  expect_error(standardize_glm(formula_outcome = Y ~ X + Z + I(X^2), formula_exposure = X ~ Z, fitbas, data = dd, values = list(X = c(-1, 0, 1))))
})
...
test_that("multiple exposure gives error when using the doubly robust estimator", {
  ...
  expect_error(standardize_glm(formula_outcome = Y ~ X + X2 + Z + I(X^2), formula_exposure = X ~ Z, fitbas, data = dd, values = list(X = c(0, 1), X2 = c(0, 1))))
})
...
test_that("check estimates and standard errors standardize_glm (simple estimator)", {
  ...
  x <- standardize_glm(formula_outcome = Y ~ X * Z, family_outcome = "binomial", data = dd, values = list(X = 0:1))
...
test_that("check estimates and standard errors standardize_glm (dr estimator)", {
  ...
  expect_equal(as.numeric(z$est_table$Estimate)[2], -223.6736, tolerance = 1e-5)
  expect_equal(as.numeric(z$est_table$`lower 0.95`)[2], -446.0778, tolerance = 1e-5)
  expect_equal(as.numeric(z$est_table$`upper 0.95`)[2], -1.269345, tolerance = 1e-5)
})
