...
cis <- list(summary(fit.std, contrast = "difference", reference = 1)$est.table[1, 3:4],
 confint(pairs(ref_grid(fit, counterfactuals = "X"))) |> (\(x){ c(x$asymp.LCL, x$asymp.UCL) })())
...
fit <- glm(formula=Y~X+Z+X*Z, family="binomial", data=dd)
fit.std <- stdGlm(fit=fit, data=dd, X="X", x=0:1)
cis <- list(summary(fit.std, contrast = "ratio", reference = 0)$est.table[2, 3:4],
      confint(ref_grid(fit, counterfactuals = "X") |> regrid(transform = "log")  |> 
        contrast("trt.vs.ctrl"), type = "response") |>  (\(x){ c(x$asymp.LCL, x$asymp.UCL) })())
