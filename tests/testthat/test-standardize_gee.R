test_that("check estimates and standard errors against older package (gee)", {
  ...
  x<-summary(fit.std)
  expect_equal(unname(x$est.table[,1]), c(-2.31169220309552, -2.02480660497755, -1.69617517072297, -1.32579790033181,
                                        ...
                                        3.88616015933548), tolerance = 1e-5)
  expect_equal(unname(x$est.table[,2]), c(0.189499305124431, 0.153122875180863, 0.124592006350899, 0.10447283361008,
                                 ...
                                 0.198889498011441), tolerance = 1e-5)
})
